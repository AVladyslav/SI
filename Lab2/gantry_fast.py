...
def build_controls():
    ...
    distance = ctrl.Antecedent(np.arange(-1, 2.1, 0.01), 'distance')

    distance['close'] = fuzz.trapmf(distance.universe, [-1, -1, 0, 0.7])
    ...
    speed['low'] = fuzz.trimf(speed.universe, [-1, 0, 1])
    speed['medium'] = fuzz.trimf(speed.universe, [0, 1, 3])
    speed['high'] = fuzz.trimf(speed.universe, [1, 3, 5])
